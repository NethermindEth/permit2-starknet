...
pub mod AllowanceTransferComponent {
    ...
    > of IAllowanceTransfer<ComponentState<TContractState>> {
        fn approve(
            ...
            let mut allowed = self.allowance.entry((caller, token, spender));
            allowed.update_amount_and_expiration(amount, expiration);
            self
                .emit(
                    events::AllowanceTransferEvent::Approval(
                        events::Approval { owner: caller, token, spender, amount, expiration },
                    ),
                )
        }
        ...
        fn invalidate_nonces(
            ...
            assert(new_nonce > old_nonce, 'Invalid Nonce');
            /// Assert delta is less than u16 max.
            assert(new_nonce - old_nonce < Bounded::<u16>::MAX.into(), 'ExcessiveInvalidation');
    ...
    > of InternalTrait<TContractState> {
        fn _transfer(
            ...
            let mut allowed = allowance_storage.read();
            /// TODO: custom error
            assert(starknet::get_block_timestamp() <= allowed.expiration, 'AllowanceExpired');
        ...
        fn _update_approval(
            ...
            let allowed = allowance_storage.read();

            /// TODO: custom error
            assert(allowed.nonce == details.nonce, 'InvalidNonce');
